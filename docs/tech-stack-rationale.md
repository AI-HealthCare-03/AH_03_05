...
### 도입 배경
프로젝트 초기 세팅에서 도입된 프레임워크입니다. 사용하면서 다음과 같은 기능을 직접 활용했습니다.
...
## 18. Jest + jest-expo (단위 테스트)

> 작성: Frontend 조이레

### 선정 배경
E2E 테스트(Playwright)는 화면 흐름을 검증하지만 `src/api/` 레이어의 엔드포인트 정확성·함수 단위 동작은 검증하지 않습니다. API 함수가 올바른 경로로 올바른 메서드를 호출하는지 단위 테스트로 보장하고, codecov를 통해 커버리지를 수치로 추적하기 위해 도입했습니다.

### 대안 비교

| 항목 | **Jest + jest-expo** | Vitest |
|------|---------------------|--------|
| Expo 공식 지원 | ✅ jest-expo 프리셋 | 별도 설정 필요 |
| RN 환경 모킹 | 프리셋 내장 | 수동 구성 |
| 생태계 성숙도 | 높음 | 비교적 신생 |
| 설정 복잡도 | 낮음 (프리셋 1줄) | 낮음 |
