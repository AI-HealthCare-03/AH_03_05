...
const STEP2_PLACEHOLDERS = ['예: 고혈압, 제2형 당뇨', '예: 페니실린, 아스피린', '예: 비타민, 오메가3'];
...
export default function OnboardingScreen({ navigation, route }: any) {
  ...
  return (
    <ScrollView
      style={styles.root}
      contentContainerStyle={[styles.container, { paddingTop: Math.max(safeTop + spacing.s8, spacing.safeTop) }, isTabletOrAbove && styles.containerDesktop]}
        ...
        <View style={styles.card}>
          ...
          {step === 1 ? (
            <>
              ...
              <View style={styles.chipRow}>
                {SEXES.map(sv => (
                  <TouchableOpacity key={sv} style={[styles.chip, styles.chipGrow, form.sex === sv && styles.chipActive]} onPress={() => set('sex', sv)}>
                    <Text style={[styles.chipText, form.sex === sv && styles.chipTextActive]}>{sv}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </>
