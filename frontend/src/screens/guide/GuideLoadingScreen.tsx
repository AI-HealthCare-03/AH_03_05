  ...
  return (
    <ScreenLayout noHeader contentStyle={{ justifyContent: 'center', alignItems: 'center' }}>
      <Card style={{ width: '90%', alignItems: 'center' }}>
        ...
        <Text style={{ fontSize: typography.fz17, fontWeight: typography.fw7, marginBottom: spacing.s1 }}>맞춤 가이드를 만들고 있어요</Text>
        <Text style={{ fontSize: typography.fz13, color: colors.muted, marginBottom: spacing.s5 }}>건강 프로필을 기준으로 분석 중입니다.</Text>

        <View style={[s.progressBg, { alignSelf: 'stretch', marginBottom: spacing.s5 }]}>
...
export function GuideResultScreen({ navigation }: any) {
  ...
  return (
    <ScreenLayout
      ...
      right={
        <Button variant="ghost" size="sm" leftIcon="chat" onPress={() => navigation.navigate('ChatTab', { screen: 'ChatList' })}>챗봇에 물어보기</Button>
      }
      scrollable
    >
      ...
      {tab === 'med' ? (
        <>
          ...
          <Card style={{ marginBottom: 14 }}>
            ...
            {warnings.map((w, i) => (
              <View key={i} style={{ flexDirection: 'row', gap: spacing.s3, marginBottom: i < warnings.length - 1 ? spacing.s3 : 0 }}>
          ...
          <Card>
            ...
            {medSteps.map((st, i) => (
              <View key={i} style={[s.scheduleRow, i > 0 && { borderTopWidth: 0.5, borderTopColor: colors.hairline }]}>
                <View style={{ width: 22, height: 22, borderRadius: radii.pill, backgroundColor: colors.accent50, alignItems: 'center', justifyContent: 'center', marginRight: spacing.s3 }}>
        ...
        <>
          <Card style={{ marginBottom: 14 }}>
            ...
            {lifeItems.map(it => (
              <TouchableOpacity key={it.id} style={{ flexDirection: 'row', gap: spacing.s3, alignItems: 'flex-start', paddingVertical: 10 }}
      ...
      <View style={{ backgroundColor: colors.danger50, borderRadius: radii.md, padding: 14, borderWidth: 1, borderColor: colors.danger, marginTop: spacing.s2, marginBottom: 14 }}>
...
const s = StyleSheet.create({
  card: { backgroundColor: colors.surface, borderRadius: radii.lg, padding: spacing.s5, borderWidth: 0.5, borderColor: colors.hairline, marginBottom: spacing.s3 },
  ...
  tabBar: { flexDirection: 'row', backgroundColor: colors.surface2, borderRadius: radii.md, padding: spacing.s1, gap: spacing.s1, marginBottom: 14, borderWidth: 0.5, borderColor: colors.hairline },
  ...
  scheduleRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: spacing.s3 },
});
