      ...
      <Card style={{ width: "90%", alignItems: "center" }}>
        ...
        {steps.map((st, i) => (
          <View key={st} style={[s.stepRow, { opacity: i > step ? 0.4 : 1 }]}>
            <View style={[s.stepDot, { backgroundColor: i < step ? colors.success : i === step ? colors.accent : colors.hairline }]}>{i < step ? <Icon name="check" size={12} color={colors.white} /> : <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: i === step ? colors.white : colors.muted2 }} />}</View>
