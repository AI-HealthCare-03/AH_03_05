    ...
    <ScreenLayout title="설정" subtitle="알림 · 보안 · 약관 · 계정 정보를 관리해요." scrollable>
      ...
      <Card style={{ marginBottom: 14 }}>
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <Button variant="ghost" size="sm" onPress={() => navigation.navigate("ProfileEdit")}>
            내 정보
          </Button>
        </View>
        <View style={{ flexDirection: "row", gap: spacing.s8, marginTop: spacing.s12 }}>
          <View style={s.chip}>
            <Text style={{ fontSize: typography.fz12, color: user.profileComplete ? colors.success : colors.muted }}>{user.profileComplete ? "건강정보 입력 완료" : "건강정보 미입력"}</Text>
          </View>
          <View style={s.chip}>
      ...
      {[
        ...
        <Card key={si} noPadding style={{ overflow: "hidden", marginBottom: 14 }}>
          {sec.map((it: any, ji) => (
            <TouchableOpacity key={it.label} style={[s.rowItem, ji > 0 && { borderTopWidth: 0.5, borderTopColor: colors.hairline }]} onPress={() => navigation.navigate(it.to, it.params)}>
      ...
      <Text style={{ fontSize: typography.fz12, color: colors.muted, textAlign: "center" }}>본 서비스는 의료 행위를 대체하지 않으며, 참고 정보를 제공합니다.</Text>
    </ScreenLayout>
  );
}
