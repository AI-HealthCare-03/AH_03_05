...
const LEGAL_DOCS: Record<
  string,
  { title: string; version: string; sections: { h: string; b: string }[] }
> = {
  tos: {
    title: '서비스 이용약관',
    version: 'v2.3 · 2026.01.01 시행',
    sections: [
      {
        h: '제1조 (목적)',
        b: '본 약관은 MediPT 서비스의 이용과 관련하여 회사와 회원 간의 권리, 의무 및 책임사항을 규정함을 목적으로 합니다.',
      },
      {
        h: '제4조 (의료 자문 대체 금지)',
        b: '본 서비스는 의료 행위가 아니며, 의사·약사 등 의료 전문가의 상담을 대체할 수 없습니다.',
      },
    ],
  },
  privacy: {
    title: '개인정보 처리방침',
    version: 'v1.7 · 2026.03.15 시행',
    sections: [
      {
        h: '1. 수집하는 개인정보 항목',
  ...
  sensitive: {
    title: '민감 건강정보 수집 · 이용 동의 내역',
    version: '최종 동의일 2026.05.01',
    sections: [
      {
        h: '수집 항목',
        b: '기저질환, 알레르기, 복용 중 약물, 처방전 이미지 및 OCR 추출 텍스트, AI 상담 대화 기록',
      },
      {
        h: '보관 기간',
        b: '회원 탈퇴 시 즉시 파기. 원본 처방전 이미지는 OCR 처리 완료 후 90일 후 자동 삭제됩니다.',
      },
    ],
  },
};
...
export function LegalDocScreen({ navigation, route }: Props) {
  ...
  return (
    <ScreenLayout
      ...
      scrollable
    >
      <Text style={{ fontSize: typography.fz12, color: colors.muted, marginBottom: spacing.s16 }}>
        {doc.version}
      </Text>
      <Card shadow>
