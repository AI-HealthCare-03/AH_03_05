...
export default function HomeScreen({ navigation }: any) {
  ...
  return (
    <View style={{ flex: 1 }}>
    <ScreenLayout noHeader scrollable scrollPadding={false} contentStyle={{ padding: spacing.s20, paddingTop: Math.max(insets.top, spacing.safeTop) + spacing.s20 }}>
        {/* 인사 + 컨디션 칩 */}
            ...
            >
              ...
              <View style={{ minHeight: 52 }}>
                {recentChat ? (
                  ...
                  <Text style={{ fontSize: typography.fz12, color: colors.muted }} numberOfLines={2}>아직 상담 내역이 없어요</Text>
                )}
              </View>
            </Card>
          </View>
        ...
        <View style={{ flexDirection: "row", gap: spacing.s12, marginBottom: spacing.s20 }}>
          ...
          <View style={{ flex: 1 }}>
            <Card
              ...
              containerStyle={{ flex: 1 }}
              style={{ flex: 1 }}
              onPress={() => navigation.getParent()?.navigate("GuideTab", { screen: "GuideResult", params: { guideId: recentGuideId } })}
            >
              <View style={s.quickIcon}>
