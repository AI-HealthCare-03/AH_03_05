...
export function DrugDosageScreen({ navigation, route }: any) {
  ...
  const save = () => {
    ...
    if (fromSearch && drugIndex === undefined) {
      ...
      setOcrSession({
        ...
        drugs: [
          ...ocrSession.drugs,
          { name: selectedDrug.drug_name, maker: selectedDrug.manufacturer ?? '', time: timeStr, confidence: 100, status: 'ok' },
        ],
      });
    } else {
      setOcrSession({
  ...
  return (
    <ScreenLayout title="복용법 입력" back onBack={() => navigation.goBack()} scrollable scrollPadding={false} contentStyle={{ padding: spacing.s20 }}>
          ...
          <View style={{ flex: 1, marginLeft: 10 }}>
            ...
            {(selectedDrug?.ingredient_name || selectedDrug?.manufacturer || existing?.maker) ? (
              <Text style={{ fontSize: typography.fz12, color: colors.successText }}>
                {selectedDrug
                  ? [selectedDrug.ingredient_name && `성분: ${selectedDrug.ingredient_name}`, selectedDrug.manufacturer && `제조사: ${selectedDrug.manufacturer}`].filter(Boolean).join(' | ')
                  : `제조사: ${existing!.maker}`}
              </Text>
            ) : null}
          </View>
