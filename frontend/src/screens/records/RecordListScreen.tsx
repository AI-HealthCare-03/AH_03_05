...
export function RecordListScreen({ navigation }: any) {
  ...
  return (
    <ScreenLayout
      ...
      right={<Button variant="primary" size="sm" leftIcon="camera" onPress={() => navigation.getParent()?.navigate('HomeTab', { screen: 'Home' })}>업로드</Button>}
      headerExtra={
    ...
    >
      {loading ? (
        ...
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: spacing.s20 }}>
          ...
          <Button variant="primary" style={{ marginTop: spacing.s16 }} onPress={() => fetchRecords()}>다시 시도</Button>
        </View>
      ) : (
        <ScrollView
          contentContainerStyle={{ padding: spacing.s16, gap: spacing.s12 }}
