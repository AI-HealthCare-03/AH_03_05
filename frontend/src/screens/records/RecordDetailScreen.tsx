...
export function RecordDetailScreen({ navigation, route }: any) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const [rec, meds] = await Promise.allSettled([recordsApi.getRecord(recordId), recordsApi.getRecordMedications(recordId)]);
    ...
    >
      ...
      <Card style={{ marginBottom: 14 }}>
        <View style={{ flexDirection: "row", gap: spacing.s16, flexWrap: "wrap" }}>
          {[["종류", typeLabel], ["업로드", formatDate(record.uploaded_at ?? "")], ["상태", record.status.replace(/_/g, " ")], ...(record.ocr_confidence != null ? [["OCR 정확도", `${Math.round(record.ocr_confidence * 100)}%`]] : [])].map(([k, v]) => (
            <View key={k}>
          ...
          medications.map((med, i) => (
            <View key={med.medication_id} style={[s.drugRow, i > 0 && { borderTopWidth: 0.5, borderTopColor: colors.hairline }]}>
              ...
              <View style={{ flex: 1, marginLeft: spacing.s12 }}>
                ...
                {med.dosage || med.frequency ? <Text style={{ fontSize: typography.fz12, color: colors.muted, marginTop: 2 }}>{[med.frequency, med.dosage].filter(Boolean).join(" · ")}</Text> : null}
              </View>
