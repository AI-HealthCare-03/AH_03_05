...
export default function Sidebar() {
  ...
  const isItemActive = (item: { tab: string; screen?: string }) => {
    ...
    // 홈 대시보드: Home 스크린이거나 GuideResult 외 화면일 때 활성
    if (item.screen === "Home" || (item.tab === "HomeTab" && !item.screen)) {
      return activeScreen !== "GuideResult";
    }
    return true;
  };
  ...
  return (
    <>
      <NotificationDrawer visible={drawerOpen} onClose={() => setDrawerOpen(false)} />
      <View style={s.sidebar}>
        {/* 브랜드 — 로고 클릭 시 홈 이동 */}
        <View style={s.brandRow}>
          <TouchableOpacity style={{ flexDirection: "row", alignItems: "center", gap: 8, flex: 1 }} onPress={() => go("HomeTab", "Home")} activeOpacity={0.7}>
            <View style={s.logo}>
