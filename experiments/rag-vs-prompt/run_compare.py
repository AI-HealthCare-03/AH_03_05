...
async def main():
    ...
    for case in cases:
        ...
        answer_text = rp["answer"].get("answer", str(rp["answer"])) if isinstance(rp["answer"], dict) else str(rp["answer"])
        ...
        answer_text = rr["answer"].get("answer", str(rr["answer"])) if isinstance(rr["answer"], dict) else str(rr["answer"])
        print(f"  [rag]    tokens={rr['prompt_tokens']} | chars={rr['context_chars']} | chunks {rr['num_chunks']}→{rr['retrieved']} | {rr['elapsed_sec']}s")
        print(f"           {answer_text[:80]}...")
